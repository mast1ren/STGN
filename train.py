...
def main():
    ...
    dataset_path = os.path.join('../../nas-public-linkdata/ds', args['dataset'])
    train_data = CrowdSeq(
        mode='train',
        path=dataset_path,
    ...
    for epoch in range(args['epochs']):
        ...
        for i, (X, density, count, seq_len) in enumerate(train_loader):
            ...
            loss = density_loss
            print(
                "\repoch:{}\t[{:>{}}/{}]\tpred: {:.2f}\tgt: {:.2f}\tloss: {:.4f}".format(
                    epoch,
                    i,
                    len(str(len(train_loader))),
                    len(train_loader),
                    torch.sum(count_pred).item() / N,
                    torch.sum(count).item() / N,
                    loss.item(),
                ),
                end='',
            )
        ...
        log = (
            '{} density loss: {:.3f} | count loss: {:.3f} | count error: {:.3f}'.format(
        ...
        #     min_mae = valid_mae
        # else:
        if valid_mae <= min_mae and epoch >= 5:
            min_mae = valid_mae
            torch.save(model.state_dict(), os.path.join(save_path, args['model_path']))
